...
test_input = 'We went in and sat down at the booth , and the waitress came over , made a big commotion over Tipper .'

test_input_clean = clean_s(strQ2B(test_input),'en')
test_input_len = len_s(test_input_clean,'en')

sp = spm.SentencePieceProcessor()
sp.load('spm8000.model')
...
encoder = SelfAttentionEncoder(src_embed=src_embed, padding_idx=src_dict.pad(), max_seq_len=1536).to(device)
decoder = SelfAttentionDecoder(tgt_embed=tgt_embed, tgt_vocab_size=len(tgt_dict),bos_idx=tgt_dict.bos(), padding_idx=tgt_dict.pad(),max_seq_len=1536).to(device)
...
subword_line = tgt_dict.string(torch.tensor(y), escape_unk=False)  # e.g. '▁我 們 ▁去 了 ...'
