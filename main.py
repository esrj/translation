...
src_dict = Dictionary.load(os.path.join('data-bin', 'spm16000', 'dict.en.txt'))
tgt_dict = Dictionary.load(os.path.join('data-bin', 'spm16000', 'dict.zh.txt'))
...
optimizer = torch.optim.Adam(
    model.parameters(),
    lr=1.0,                 # base LR；實際 LR = base_lr * noam_lambda(...)
    betas=(0.9, 0.98),
    eps=1e-9,
    weight_decay=1e-4
)
